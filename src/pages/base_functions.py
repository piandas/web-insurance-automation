...
class BasePage:
    ...
    def __init__(self, page: Page):
        self.page = page
        # Pre-configuramos el frame locator para no repetir la cadena
    ...
    async def wait_for_element_with_text(self, text: str, timeout: int = 30000) -> bool:
        ...
        lower = text.lower()
        print(f"⏳ Esperando texto '{text}' (<= {timeout/1000}s)...")
        fn = f"""
        () => {{
            const t = '{lower}';
    ...
    async def wait_for_element_by_id_in_iframe(self, element_id: str, timeout: int = 30000) -> bool:
        ...
        sel = f"#{element_id}"
        print(f"⏳ Esperando '{sel}' en iframe (<= {timeout/1000}s)...")
        try:
            await self.page.wait_for_function(
            ...
            print(f"✅ Elemento '{sel}' en iframe encontrado")
            return True
        except Exception as e:
            print(f"❌ Timeout esperando '{sel}' en iframe: {e}")
            return False
    ...
    async def wait_for_load_state_with_retry(self, state: str = "networkidle", timeout: int = 30000):
        ...
        try:
            ...
            print(f"⚠️ Timeout load_state='{state}', continuando...")
    ...
    async def wait_for_selector_safe(self, selector: str, state: str = "visible", timeout: int = 15000) -> bool:
        ...
        print(f"⏳ Esperando selector '{selector}' ({state}) <={timeout/1000}s...")
        try:
            await self.page.wait_for_selector(selector, state=state, timeout=timeout)
            print(f"✅ Selector '{selector}' {state}")
            return True
        except Exception as e:
            print(f"❌ wait_for_selector_safe('{selector}'): {e}")
            return False
    ...
    async def click_in_frame(self, selector: str, description: str, timeout: int = 15000) -> bool:
        ...
        print(f"⏳ Esperando {description} en iframe...")
        try:
            el = self._frame.locator(selector)
            await el.wait_for(timeout=timeout)
            await el.click()
            print(f"✅ Clic en {description} exitoso!")
            return True
        except Exception as e:
            print(f"❌ click_in_frame('{selector}'): {e}")
            return False
    ...
    async def fill_in_frame(self, selector: str, value: str, description: str, timeout: int = 15000) -> bool:
        ...
        print(f"⏳ Esperando campo {description} en iframe...")
        try:
            el = self._frame.locator(selector)
            await el.wait_for(timeout=timeout)
            await el.fill(value)
            print(f"✅ Campo {description} = '{value}'")
            return True
        except Exception as e:
            print(f"❌ fill_in_frame('{selector}'): {e}")
            return False
    ...
    async def select_in_frame(self, selector: str, value: str, description: str, timeout: int = 15000) -> bool:
        ...
        print(f"⏳ Esperando dropdown {description} en iframe...")
        try:
            el = self._frame.locator(selector)
            await el.wait_for(timeout=timeout)
            ...
            print(f"✅ {description} seleccionado: {value}")
            return True
        except Exception as e:
            print(f"❌ select_in_frame('{selector}'): {e}")
            return False
